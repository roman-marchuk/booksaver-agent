...
async def _current_visible_saved_reservation(
    ...
    await remember_visible_semantic_state(browser_session, snapshots)
    matches: dict[str, KnownInventoryReservation] = {}
    for visible_dom in reversed(snapshots):
        ...
        normalized_scopes.append(
            scope_item.model_copy(
                update={
                    ...
                    "requested_scope_visible": normalize_provider_tri_state(
                        scope_item.requested_scope_visible
                    ),
                    "explicit_empty": normalize_provider_tri_state(
                        scope_item.explicit_empty
                    ),
                    "pagination_exhausted": normalize_provider_tri_state(
...
class LocalBrowserUseInventoryRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        self._host = BrowserUseSessionHost(self._mobile_settings)
    ...
    async def execute(
        ...
        *,
        ...
        async def guarded_key(  # type: ignore[no-untyped-def]
            ...
            if params.key not in SAFE_KEYS or not await before_action(browser_session):
        ...
        if self._state.observation is None:
            ...
            return BrowserUseRuntimeResult(status)
        self._host.failure_stage = "result_mapping"
        try:
...
class BrowserUseInventoryBrowserExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        runtime_factory: Callable[[], BrowserUseRuntimePort] = LocalBrowserUseInventoryRuntime,
    ) -> None:
    ...
    def execute(self, request: InventoryExecutionRequest) -> InventoryExecutionResult:
        with AsyncLoopRunner() as runner:
            return BrowserUseInventoryBrowserExecutor(
                ...
                budget=self._budget,
                runner=runner,
                runtime_factory=lambda: LocalBrowserUseInventoryRuntime(
